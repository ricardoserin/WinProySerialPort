...
namespace winproySerialPort
{
    class TramasEnvio
    {
        ...
        public Trama SiguienteTrama()
        {
            ...
            {
                IndiceTrama++;
                if (IndiceTrama == NumeroDeTramas + 1) Enviado = true;
            }
        ...
        private byte[] GenerarCabeceraInformacion()
        {
            var headerPrefix = (TipoDatos == ETipoTrama.Archivo)
                ? "AI"
                : "MI";
            var stringHeader = headerPrefix + IdEnvio.ToString("D3");
        ...
        private Trama GenerarTramaCabecera(ETipoTrama tipo, string[] metadatos)
        {
            ...
            var bytesCuerpo = Encoding.UTF8.GetBytes(cuerpo);
            var bytes = Encoding.UTF8.GetBytes(cabecera).Concat(bytesCuerpo).ToArray();
            if (bytesCuerpo.Length < 1019)
            {
                var tamanoRelleno = 1019 - bytesCuerpo.Length;
                var relleno = new byte[tamanoRelleno];
                for (short i = 0; i < tamanoRelleno; i++)
                {
                    relleno[i] = 64;
